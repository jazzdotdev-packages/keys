...
if not response.headers.date then
  response.headers.date = tostring(time.now())
end

-- Fails if body is not a string
local signature_string = "date: " .. response.headers.date .. "\n" .. response.body

local signature = priv_key:sign_detached(signature_string)

response.headers["x-profile-uuid"] = profile_uuid
response.headers.signature = 'keyId="' .. profile_uuid .. '",algorithm="ed25519",signature="' .. signature .. '"'
